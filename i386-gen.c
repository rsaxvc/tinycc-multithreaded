...
{
    if (r & VT_SYM)
        greloc(s1, cur_text_section, sym, s1->ind, R_386_32);
    gen_le32(s1, c);
}
...
{
    if (r & VT_SYM)
        greloc(s1, cur_text_section, sym, s1->ind, R_386_PC32);
    gen_le32(s1, c - 4);
}
...
ST_FUNC void load(TCCState *s1, int r, SValue *sv)
{
    ...
    if (fr & VT_LVAL) {
        ...
        if (v == VT_CONST) {
            o(s1, 0xb8 + r); /* mov $xx, r */
            gen_addr32(s1, fr, sv->sym, fc);
        } else if (v == VT_LOCAL) {
            if (fc) {
                o(s1, 0x8d); /* lea xxx(%ebp), r */
                gen_modrm(s1, r, VT_LOCAL, sv->sym, fc);
            } else {
                o(s1, 0x89);
                o(s1, 0xe8 + r); /* mov %ebp, r */
            }
        } else if (v == VT_CMP) {
            o(s1, 0x0f); /* setxx %br */
            o(s1, fc);
            o(s1, 0xc0 + r);
            o(s1, 0xc0b60f + r * 0x90000); /* movzbl %al, %eax */
        } else if (v == VT_JMP || v == VT_JMPI) {
            t = v & 1;
            oad(s1, 0xb8 + r, t); /* mov $1, r */
            o(s1, 0x05eb); /* jmp after */
            gsym(s1, fc);
            oad(s1, 0xb8 + r, t ^ 1); /* mov $0, r */
        } else if (v != r) {
            o(s1, 0x89);
            o(s1, 0xc0 + r + v * 8); /* mov v, r */
        }
    }
}
...
static void gen_static_call(TCCState *s1, int v)
{
    ...
    sym = external_helper_sym(s1, v);
    oad(s1, 0xe8, -4);
    greloc(s1, cur_text_section, sym, s1->ind-4, R_386_PC32);
}
#endif
...
static void gcall_or_jmp(TCCState *s1, int is_jmp)
{
    ...
    if ((s1->vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST && (s1->vtop->r & VT_SYM)) {
        ...
        greloc(s1, cur_text_section, s1->vtop->sym, s1->ind + 1, R_386_PC32);
        oad(s1, 0xe8 + is_jmp, s1->vtop->c.i - 4); /* call/jmp im */
    } else {
        /* otherwise, indirect call */
        r = gv(s1, RC_INT);
        o(s1, 0xff); /* call/jmp *r */
        o(s1, 0xd0 + r + (is_jmp << 4));
    }
}
...
ST_FUNC void gfunc_call(TCCState *s1, int nb_args)
{
    ...
    for(i = 0;i < nb_args; i++) {
        if ((s1->vtop->type.t & VT_BTYPE) == VT_STRUCT) {
            ...
            if (size >= 4096) {
                r = get_reg(s1, RC_EAX);
                oad(s1, 0x68, size); // push size
                /* cannot call normal 'alloca' with bound checking */
                gen_static_call(s1, tok_alloc_const(s1, "__alloca"));
                gadd_sp(s1, 4);
            } else
#endif
            {
                oad(s1, 0xec81, size); /* sub $xxx, %esp */
                /* generate structure store */
                r = get_reg(s1, RC_INT);
                o(s1, 0xe089 + (r << 8)); /* mov %esp, r */
            }
            ...
            gv(s1, RC_FLOAT); /* only one float register */
            ...
            if (size == 12)
                ...
                o(s1, 0x5cd9 + size - 4); /* fstp[s|l] 0(%esp) */
            g(s1, 0x24);
            g(s1, 0x00);
            args_size += size;
        } else {
    ...
    if ((func_call >= FUNC_FASTCALL1 && func_call <= FUNC_FASTCALL3) ||
        ...
        for(i = 0;i < fastcall_nb_regs; i++) {
            if (args_size <= 0)
                break;
            o(s1, 0x58 + fastcall_regs_ptr[i]); /* pop r */
            /* XXX: incorrect for struct/floats */
            args_size -= 4;
        }
    }
#if !defined(TCC_TARGET_PE) && !TARGETOS_FreeBSD || TARGETOS_OpenBSD
    else if ((s1->vtop->type.ref->type.t & VT_BTYPE) == VT_STRUCT)
        args_size -= 4;
#endif
...
ST_FUNC void gfunc_prolog(TCCState *s1, Sym *func_sym)
{
    ...
    while ((sym = sym->next) != NULL) {
        ...
        if (param_index < fastcall_nb_regs) {
            ...
            s1->loc -= 4;
            o(s1, 0x89);     /* movl */
            gen_modrm(s1, fastcall_regs_ptr[param_index], VT_LOCAL, NULL, s1->loc);
            param_addr = s1->loc;
        } else {
            param_addr = addr;
            addr += size;
...
ST_FUNC void gfunc_epilog(TCCState *s1)
{
...
#if USE_EBX
    o(s1, 0x8b);
    gen_modrm(s1, TREG_EBX, VT_LOCAL, NULL, -(v+4));
#endif
    ...
    if (v >= 4096) {
        ...
        gen_static_call(s1, TOK___chkstk); /* call __chkstk, (does the stackframe too) */
    } else
#endif
    {
        o(s1, 0xe58955);  /* push %ebp, mov %esp, %ebp */
        o(s1, 0xec81);  /* sub esp, stacksize */
        gen_le32(s1, v);
#ifdef TCC_TARGET_PE
        o(s1, 0x90);  /* adjust to FUNC_PROLOG_SIZE */
#endif
    }
    o(s1, 0x53 * USE_EBX); /* push ebx */
    s1->ind = saved_ind;
}
...
#if 0
/* generate a jump to a fixed address */
ST_FUNC void gjmp_cond_addr(TCCState *s1, int a, int op)
{
    int r = a - s1->ind - 2;
    if (r == (char)r)
        g(s1, op - 32), g(s1, r);
    else
        g(s1, 0x0f), gjmp2(s1, op - 16, r - 4);
}
#endif
...
ST_FUNC void gen_opi(TCCState *s1, int op)
{
    ...
    gen_shift:
        ...
        if ((s1->vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
            ...
            vswap(s1);
            c = s1->vtop->c.i & 0x1f;
            o(s1, 0xc1); /* shl/shr/sar $xxx, r */
            o(s1, opc | r);
            g(s1, c);
        } else {
            /* we generate the shift in ecx */
            gv2(s1, RC_INT, RC_ECX);
            r = s1->vtop[-1].r;
            o(s1, 0xd3); /* shl/shr/sar %cl, r */
            o(s1, opc | r);
        }
        s1->vtop--;
        break;
    ...
    case TOK_UMULL:
        ...
        gv2(s1, RC_EAX, RC_ECX);
        r = s1->vtop[-1].r;
        fr = s1->vtop[0].r;
        s1->vtop--;
        save_reg(s1, TREG_EDX);
        /* save EAX too if used otherwise */
        save_reg_upstack(s1, TREG_EAX, 1);
        if (op == TOK_UMULL) {
            o(s1, 0xf7); /* mul fr */
            o(s1, 0xe0 + fr);
            s1->vtop->r2 = TREG_EDX;
            ...
            if (op == TOK_UDIV || op == TOK_UMOD) {
                o(s1, 0xf7d231); /* xor %edx, %edx, div fr, %eax */
                o(s1, 0xf0 + fr);
            } else {
                o(s1, 0xf799); /* cltd, idiv fr, %eax */
                o(s1, 0xf8 + fr);
            }
            if (op == '%' || op == TOK_UMOD)
                r = TREG_EDX;
...
ST_FUNC void gen_opf(TCCState *s1, int op)
{
    ...
    if (op >= TOK_ULT && op <= TOK_GT) {
        ...
        save_reg(s1, TREG_EAX); /* eax is used by FP comparison code */
        ...
        if ((ft & VT_BTYPE) == VT_LDOUBLE) {
            ...
            if ((ft & VT_BTYPE) == VT_DOUBLE)
                ...
                o(s1, 0xd8);
            gen_modrm(s1, a, r, s1->vtop->sym, fc);
        }
        s1->vtop--;
    }
}
...
ST_FUNC void gen_cvt_itof(TCCState *s1, int t)
{
    ...
    if ((s1->vtop->type.t & VT_BTYPE) == VT_LLONG) {
        ...
        o(s1, 0x50 + s1->vtop->r2); /* push r2 */
        o(s1, 0x50 + (s1->vtop->r & VT_VALMASK)); /* push r */
        o(s1, 0x242cdf); /* fildll (%esp) */
        o(s1, 0x08c483); /* add $8, %esp */
        s1->vtop->r2 = VT_CONST;
        ...
        o(s1, 0x50 + (s1->vtop->r & VT_VALMASK)); /* push r */
        o(s1, 0x2404db); /* fildl (%esp) */
        o(s1, 0x04c483); /* add $4, %esp */
    }
    s1->vtop->r2 = VT_CONST;
    s1->vtop->r = TREG_ST0;
}
...
ST_FUNC void gen_cvt_ftoi(TCCState *s1, int t)
{
    ...
    if (bt == VT_FLOAT)
        vpush_helper_func(s1, TOK___fixsfdi);
    else if (bt == VT_LDOUBLE)
        vpush_helper_func(s1, TOK___fixxfdi);
    else
        vpush_helper_func(s1, TOK___fixdfdi);
    vswap(s1);
    gfunc_call(s1, 1);
    vpushi(s1, 0);
    s1->vtop->r = REG_IRET;
...
{
   o(s1, 0x0583); /* addl $1, xxx */
   greloc(s1, cur_text_section, sv->sym, s1->ind, R_386_32);
   gen_le32(s1, 0);
   o(s1, 1);
   o(s1, 0x1583); /* addcl $0, xxx */
   greloc(s1, cur_text_section, sv->sym, s1->ind, R_386_32);
   gen_le32(s1, 4);
   g(s1, 0);
}
...
static void gen_bounds_epilog(TCCState *s1)
{
    ...
    *bounds_ptr = 0;
    ...
    if (offset_modified) {
        ...
        greloc(s1, cur_text_section, sym_data, s1->ind + 1, R_386_32);
        s1->ind = s1->ind + 5;
...
ST_FUNC void gen_vla_sp_save(TCCState *s1, int addr) {
    /* mov %esp,addr(%ebp)*/
    o(s1, 0x89);
    gen_modrm(s1, TREG_ESP, VT_LOCAL, NULL, addr);
}

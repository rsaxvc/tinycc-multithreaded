...
static int asm2cname(TCCState *s1, int v, int *addeddot)
{
    ...
    *addeddot = 0;
    if (!s1->leading_underscore)
      return v;
    name = get_tok_str(s1, v, NULL);
    if (!name)
      return v;
    if (name[0] == '_') {
        v = tok_alloc_const(s1, name + 1);
    } else if (!strchr(name, '.')) {
        char newname[256];
        snprintf(newname, sizeof newname, ".%s", name);
        v = tok_alloc_const(s1, newname);
        *addeddot = 1;
    }
    return v;
}
...
static Sym *asm_label_push(TCCState *s1, int v)
{
    ...
    /* We always add VT_EXTERN, for sym definition that's tentative
       ...
       as is.  */
    Sym *sym = global_identifier_push(s1, v2, VT_ASM | VT_EXTERN | VT_STATIC, 0);
    if (addeddot)
        sym->asm_label = v;
    return sym;
}
        ...
        if (*p == 'b' || *p == 'f') {
            ...
            if (*p == 'b') {
                ...
                if (sym && (!sym->c || elfsym(s1, sym)->st_shndx == SHN_UNDEF))
                    sym = sym->prev_tok;
                if (!sym)
                    tcc_error(s1, "local label '%d' not found backward", (int)n);
            } else {
                /* forward */
                if (!sym || (sym->c && elfsym(s1, sym)->st_shndx != SHN_UNDEF)) {
                           ...
                           int sh_num, int value)
{
    ...
    if (!sym->c)
      put_extern_sym2(s1, sym, SHN_UNDEF, 0, 0, 1);
    esym = elfsym(s1, sym);
...
static Sym* asm_new_label(TCCState *s1, int label, int is_local)
{
    return asm_new_label1(s1, label, is_local, cur_text_section->sh_num, s1->ind);
}
...
static void asm_parse_directive(TCCState *s1, int global)
{
    ...
    case TOK_ASMDIR_section:
	    ...
	    tok1 = s1->tok;
            ...
            s1->last_text_section = cur_text_section;
...
ST_FUNC void asm_instr(TCCState *s1)
{
    ...
    if (sec != cur_text_section) {
        tcc_warning(s1, "inline asm tries to change current section");
        use_section1(s1, sec);
    }

...
static void parse_operand(TCCState *s1, Operand *op)
{
    ...
    if (s1->tok == '{') { // regset literal
        next(s1); // skip '{'
        while (s1->tok != '}' && s1->tok != TOK_EOF) {
            reg = asm_parse_regvar(s1, s1->tok);
            if (reg == -1) {
                expect(s1, "register");
                return;
            } else
                next(s1); // skip register name

            if ((1 << reg) < regset)
                tcc_warning(s1, "registers will be processed in ascending order by hardware--but are not specified in ascending order here");
            regset |= 1 << reg;
            if (s1->tok != ',')
                break;
            next(s1); // skip ','
        ...
        if (regset == 0) {
            ...
            tcc_error(s1, "empty register list is not supported");
        } else {
            op->type = OP_REGSET32;
            op->regset = regset;
        }
        return;
    } else if ((reg = asm_parse_regvar(s1, s1->tok)) != -1) {
        next(s1); // skip register name
        op->type = OP_REG32;
        op->reg = (uint8_t) reg;
        return;
    } else if ((reg = asm_parse_vfp_regvar(s1->tok, 0)) != -1) {
        next(s1); // skip register name
        op->type = OP_VREG32;
        op->reg = (uint8_t) reg;
        return;
    } else if ((reg = asm_parse_vfp_regvar(s1->tok, 1)) != -1) {
        next(s1); // skip register name
        op->type = OP_VREG64;
        op->reg = (uint8_t) reg;
        return;
    } else if (s1->tok == '#' || s1->tok == '$') {
        /* constant value */
        next(s1); // skip '#' or '$'
    }
    asm_expr(s1, &e);
...
ST_FUNC void gen_le32 (TCCState *s1, int i)
{
    ...
    if (ind1 > cur_text_section->data_allocated)
        section_realloc(cur_text_section, ind1);
    cur_text_section->data[s1->ind++] = i & 0xFF;
    cur_text_section->data[s1->ind++] = (i >> 8) & 0xFF;
    cur_text_section->data[s1->ind++] = (i >> 16) & 0xFF;
    cur_text_section->data[s1->ind++] = (i >> 24) & 0xFF;
}
...
static uint32_t condition_code_of_token(TCCState *s1, int token) {
    if (token < TOK_ASM_nopeq) {
        expect(s1, "condition-enabled instruction");
        return 0;
    } else
        return (token - TOK_ASM_nopeq) & 15;
}

static void asm_emit_opcode(TCCState *s1, int token, uint32_t opcode) {
    gen_le32(s1, (condition_code_of_token(s1, token) << 28) | opcode);
}

static void asm_emit_unconditional_opcode(TCCState *s1, uint32_t opcode) {
    gen_le32(s1, opcode);
}

static void asm_emit_coprocessor_opcode(TCCState *s1, uint32_t high_nibble, uint8_t cp_number, uint8_t cp_opcode, uint8_t cp_destination_register, uint8_t cp_n_operand_register, uint8_t cp_m_operand_register, uint8_t cp_opcode2, int inter_processor_transfer)
{
    ...
    asm_emit_unconditional_opcode(s1, (high_nibble << 28) | opcode);
}
...
static void asm_nullary_opcode(TCCState *s1, int token)
{
    ...
    case TOK_ASM_nopeq:
        asm_emit_opcode(s1, token, 0xd << 21); // mov r0, r0
        break;
    case TOK_ASM_wfeeq:
        asm_emit_opcode(s1, token, 0x320f002);
    case TOK_ASM_wfieq:
        asm_emit_opcode(s1, token, 0x320f003);
        break;
    default:
        expect(s1, "nullary instruction");
    }
}
...
static void asm_unary_opcode(TCCState *s1, int token)
{
    ...
    case TOK_ASM_svceq:
        if (op.type != OP_IM8)
            expect(s1, "immediate 8-bit unsigned integer");
        else {
            /* Note: Dummy operand (ignored by processor): ARM ref documented 0...255, ARM instruction set documented 24 bit */
            asm_emit_opcode(s1, token, (0xf << 24) | op.e.v);
        }
        break;
    default:
        expect(s1, "unary instruction");
    }
}
...
static void asm_binary_opcode(TCCState *s1, int token)
{
    ...
    if (ops[0].type != OP_REG32) {
        expect(s1, "(destination operand) register");
        return;
    }

    if (ops[0].reg == 15) {
        tcc_error(s1, "'%s' does not support 'pc' as operand", get_tok_str(s1, token, NULL));
        return;
    }

    if (ops[0].reg == 13)
        tcc_warning(s1, "Using 'sp' as operand with '%s' is deprecated by ARM", get_tok_str(s1, token, NULL));
        ...
        case TOK_ASM_movweq:
            if (ops[1].type == OP_IM8 || ops[1].type == OP_IM8N || ops[1].type == OP_IM32) {
                if (ops[1].e.v >= 0 && ops[1].e.v <= 0xFFFF) {
                    ...
                    switch (ARM_INSTRUCTION_GROUP(token)) {
                    case TOK_ASM_movteq:
                        asm_emit_opcode(s1, token, 0x3400000 | (ops[0].reg << 12) | (immediate_value & 0xF000) << 4 | (immediate_value & 0xFFF));
                        break;
                    case TOK_ASM_movweq:
                        asm_emit_opcode(s1, token, 0x3000000 | (ops[0].reg << 12) | (immediate_value & 0xF000) << 4 | (immediate_value & 0xFFF));
                        break;
                    }
                } else
                    expect(s1, "(source operand) immediate 16 bit value");
            } else
                expect(s1, "(source operand) immediate");
            break;
        default:
            expect(s1, "(source operand) register");
        }
        return;
    }
    ...
    if (s1->tok == ',') {
        next(s1); // skip ','
        if (s1->tok == TOK_ASM_ror) {
            next(s1); // skip 'ror'
            parse_operand(s1, &rotation);
            if (rotation.type != OP_IM8) {
                expect(s1, "immediate value for rotation");
                return;
            } else {
                amount = rotation.e.v;
                switch (amount) {
    ...
    case TOK_ASM_clzeq:
        if (encoded_rotation)
            tcc_error(s1, "clz does not support rotation");
        asm_emit_opcode(s1, token, 0x16f0f10 | (ops[0].reg << 12) | ops[1].reg);
        break;
    case TOK_ASM_sxtbeq:
        asm_emit_opcode(s1, token, 0x6af0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    case TOK_ASM_sxtheq:
        asm_emit_opcode(s1, token, 0x6bf0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    case TOK_ASM_uxtbeq:
        asm_emit_opcode(s1, token, 0x6ef0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    case TOK_ASM_uxtheq:
        asm_emit_opcode(s1, token, 0x6ff0070 | (ops[0].reg << 12) | ops[1].reg | encoded_rotation);
        break;
    default:
        expect(s1, "binary instruction");
    }
}
...
static void asm_coprocessor_opcode(TCCState *s1, int token) {
    ...
    if (s1->tok >= TOK_ASM_p0 && s1->tok <= TOK_ASM_p15) {
        coprocessor = s1->tok - TOK_ASM_p0;
        next(s1);
    } else {
        expect(s1, "'p<number>'");
        return;
    ...
    if (opcode1.type != OP_IM8 || opcode1.e.v > 15) {
        tcc_error(s1, "opcode1 of instruction '%s' must be an immediate value between 0 and 15", get_tok_str(s1, token, NULL));
        return;
    }
    ...
    for (i = 0; i < 3; ++i) {
        ...
        if (i == 0 && token != TOK_ASM_cdp2 && (ARM_INSTRUCTION_GROUP(token) == TOK_ASM_mrceq || ARM_INSTRUCTION_GROUP(token) == TOK_ASM_mcreq)) {
            if (s1->tok >= TOK_ASM_r0 && s1->tok <= TOK_ASM_r15) {
                registers[i] = s1->tok - TOK_ASM_r0;
                next(s1);
            } else {
                expect(s1, "'r<number>'");
                return;
            ...
            if (s1->tok >= TOK_ASM_c0 && s1->tok <= TOK_ASM_c15) {
                registers[i] = s1->tok - TOK_ASM_c0;
                next(s1);
            } else {
                expect(s1, "'c<number>'");
                return;
    ...
    if (opcode2.type != OP_IM8 || opcode2.e.v > 15) {
        tcc_error(s1, "opcode2 of instruction '%s' must be an immediate value between 0 and 15", get_tok_str(s1, token, NULL));
        return;
    }

    if (token == TOK_ASM_cdp2) {
        high_nibble = 0xF;
        asm_emit_coprocessor_opcode(s1, high_nibble, coprocessor, opcode1.e.v, registers[0], registers[1], registers[2], opcode2.e.v, 0);
        return;
    } else
        high_nibble = condition_code_of_token(s1, token);
    ...
    case TOK_ASM_mcreq:
        // opcode1 encoding changes! highest and lowest bit gone.
        if (opcode1.e.v > 7) {
            tcc_error(s1, "opcode1 of instruction '%s' must be an immediate value between 0 and 7", get_tok_str(s1, token, NULL));
            return;
        }
        asm_emit_coprocessor_opcode(s1, high_nibble, coprocessor, (opcode1.e.v << 1) | mrc, registers[0], registers[1], registers[2], opcode2.e.v, 1);
        break;
    default:
        expect(s1, "known instruction");
    }
}
...
static void asm_block_data_transfer_opcode(TCCState *s1, int token)
{
    ...
    if (nb_ops < 1) {
        expect(s1, "at least one operand");
        return;
    } else if (ops[nb_ops - 1].type != OP_REGSET32) {
        expect(s1, "(last operand) register list");
        return;
    }
    ...
    case TOK_ASM_pusheq: // TODO: Optimize 1-register case to: str ?, [sp, #-4]!
        ...
        //      Register List: bits 15...0
        if (nb_ops != 1)
            expect(s1, "exactly one operand");
        else
            asm_emit_opcode(s1, token, (0x92d << 16) | ops[0].regset); // TODO: base register ?
        break;
    case TOK_ASM_popeq: // TODO: Optimize 1-register case to: ldr ?, [sp], #4
        ...
        if (nb_ops != 1)
            ...
            asm_emit_opcode(s1, token, (0x8bd << 16) | ops[0].regset); // TODO: base register ?
        break;
    ...
    case TOK_ASM_ldmibeq:
        ...
        case TOK_ASM_ldmibeq: // pre-increment load
            ...
            break;
        default:
            tcc_error(s1, "internal error: This place should not be reached (fallback in asm_block_data_transfer_opcode)");
        }
        // operands:
        //    Rn: first operand
        //    Register List: lower bits
        if (nb_ops != 2)
            expect(s1, "exactly two operands");
        else if (ops[0].type != OP_REG32)
            expect(s1, "(first operand) register");
        else {
            if (op0_exclam)
                opcode |= 1 << 21; // writeback
            asm_emit_opcode(s1, token, opcode | ENCODE_RN(ops[0].reg) | ops[1].regset);
        }
        break;
    default:
        expect(s1, "block data transfer instruction");
    }
}
...
static uint32_t asm_encode_shift(TCCState *s1, Operand* shift)
{
    uint64_t amount;
    uint32_t operands = 0;
    switch (shift->type) {
    case OP_REG32:
        if (shift->reg == 15)
            tcc_error(s1, "r15 cannot be used as a shift count");
        else {
    ...
    case OP_IM8:
        ...
        if (amount > 0 && amount < 32)
            ...
            tcc_error(s1, "shift count out of range");
        break;
    default:
        tcc_error(s1, "unknown shift amount");
    }
    return operands;
}
...
static void asm_data_processing_opcode(TCCState *s1, int token)
{
    ...
    for (nb_ops = 0; nb_ops < sizeof(ops)/sizeof(ops[0]); ) {
        if (s1->tok == TOK_ASM_asl || s1->tok == TOK_ASM_lsl || s1->tok == TOK_ASM_lsr || s1->tok == TOK_ASM_asr || s1->tok == TOK_ASM_ror || s1->tok == TOK_ASM_rrx)
            break;
    ...
    if (nb_ops < 2)
        expect(s1, "at least two operands");
    else if (nb_ops == 2) {
        ...
        if (opcode_nos == 0xd || opcode_nos == 0xf || opcode_nos == 0xa || opcode_nos == 0xb || opcode_nos == 0x8 || opcode_nos == 0x9) { // mov, mvn, cmp, cmn, tst, teq
            tcc_error(s1, "'%s' cannot be used with three operands", get_tok_str(s1, token, NULL));
            return;
        }
    }
    if (nb_ops != 3) {
        expect(s1, "two or three operands");
        return;
    } else {
        uint32_t opcode = 0;
        ...
        if (nb_shift && shift.type == OP_REG32) {
            if ((ops[0].type == OP_REG32 && ops[0].reg == 15) ||
                ...
                tcc_error(s1, "Using the 'pc' register in data processing instructions that have a register-controlled shift is not implemented by ARM");
                return;
            }
        }
        ...
        if (ops[1].type != OP_REG32)
            expect(s1, "(first source operand) register");
        ...
        case OP_IM8N: // immediate negative value
            ...
            */
            ...
            case 0xf: // MVN - Rd:= NOT Op2
                ...
                break;
            default:
                tcc_error(s1, "cannot use '%s' with a negative immediate value", get_tok_str(s1, token, NULL));
            }
            for (half_immediate_rotation = 0; half_immediate_rotation < 16; ++half_immediate_rotation) {
                if (immediate_value >= 0x00 && immediate_value < 0x100)
                    break;
            ...
            if (half_immediate_rotation >= 16) {
                immediate_value = ops[2].e.v;
                tcc_error(s1, "immediate value 0x%X cannot be encoded into ARM immediate", (unsigned) immediate_value);
                return;
            }
            operands |= immediate_value;
            operands |= half_immediate_rotation << 8;
            break;
        default:
            expect(s1, "(second source operand) register or immediate value");
        }

        if (nb_shift) {
            if (operands & ENCODE_IMMEDIATE_FLAG)
                tcc_error(s1, "immediate rotation not implemented");
            else
                operands |= asm_encode_shift(s1, &shift);
        }

        /* S=0 and S=1 entries alternate one after another, in that order */
        opcode |= (opcode_idx & 1) ? ENCODE_SET_CONDITION_CODES : 0;
        asm_emit_opcode(s1, token, opcode | operands);
    }
}
...
static void asm_shift_opcode(TCCState *s1, int token)
{
    ...
    if (nb_ops == 2) {
        ...
        case TOK_ASM_rrxeq:
            if (ops[1].type == OP_REG32) {
                ...
                tcc_error(s1, "(first source operand) register");
            return;
        default:
            memcpy(&ops[2], &ops[1], sizeof(ops[1])); // move ops[2]
            memcpy(&ops[1], &ops[0], sizeof(ops[0])); // ops[1] was implicit
    ...
    case OP_IM8:
        operands |= ENCODE_IMMEDIATE_FLAG;
        operands |= ops[1].e.v;
        tcc_error(s1, "Using an immediate value as the source operand is not possible with '%s' instruction on ARM", get_tok_str(s1, token, NULL));
        return;
    }
    ...
    case OP_REG32:
        ...
        operands |= asm_encode_shift(s1, &ops[2]);
        break;
    case OP_IM8:
        if (ops[2].e.v)
            operands |= asm_encode_shift(s1, &ops[2]);
...
static void asm_multiplication_opcode(TCCState *s1, int token)
{
    ...
    if (nb_ops < 2)
        ...
        case TOK_ASM_muleq:
            ...
            break;
        default:
            expect(s1, "at least three operands");
            return;
        }
        nb_ops = 3;
    }
    ...
    if (ops[2].type == OP_REG32)
        ...
        expect(s1, "(second source operand) register");
    if (nb_ops > 3) {
        if (ops[3].type == OP_REG32)
            opcode |= ops[3].reg << 12;
        else
            expect(s1, "(third source operand) register");
    }
    ...
    case TOK_ASM_mlaeq:
        ...
        break;
    default:
        expect(s1, "known multiplication instruction");
    }
}
...
static void asm_long_multiplication_opcode(TCCState *s1, int token)
{
    ...
    if (ops[0].type == OP_REG32)
        ...
        expect(s1, "(destination lo accumulator) register");
    if (ops[1].type == OP_REG32)
        opcode |= ops[1].reg << 16;
    else
        expect(s1, "(destination hi accumulator) register");
    if (ops[2].type == OP_REG32)
        opcode |= ops[2].reg;
    else
        expect(s1, "(first source operand) register");
    ...
    case TOK_ASM_umlaleq:
        ...
        break;
    default:
        expect(s1, "known long multiplication instruction");
    }
}
...
static void asm_single_data_transfer_opcode(TCCState *s1, int token)
{
    ...
    if (s1->tok != ',')
        expect(s1, "at least two arguments");
    else
        next(s1); // skip ','
    ...
    case TOK_ASM_strexeq:
        ...
        if (s1->tok != ',')
            expect(s1, "at least three arguments");
        else
            next(s1); // skip ','
        break;
    }
    ...
    if (s1->tok == ',') {
        ...
        if (ops[2].type == OP_REG32) {
            if (ops[2].reg == 15) {
                tcc_error(s1, "Using 'pc' for register offset in '%s' is not implemented by ARM", get_tok_str(s1, token, NULL));
                return;
            }
            if (s1->tok == ',') {
                next(s1);
                ...
                if (opcode == 0)
                    expect(s1, "shift directive, or no comma");
            }
        }
    } else {
    ...
    if (ops[2].type == OP_IM32 || ops[2].type == OP_IM8 || ops[2].type == OP_IM8N) {
        int v = ops[2].e.v;
        if (op2_minus)
            tcc_error(s1, "minus before '#' not supported for immediate values");
        if (v >= 0) {
            opcode |= 1 << 23; // up
            if (v >= 0x1000)
                tcc_error(s1, "offset out of range for '%s'", get_tok_str(s1, token, NULL));
            else
                opcode |= v;
        } else { // down
            if (v <= -0x1000)
    ...
    case TOK_ASM_streq:
        ...
        if (nb_shift)
            opcode |= asm_encode_shift(s1, &shift);
        asm_emit_opcode(s1, token, opcode);
        break;
    case TOK_ASM_ldrbeq:
        opcode |= 1 << 22; // B
    ...
    case TOK_ASM_strexeq:
        if ((opcode & 0xFFF) || nb_shift) {
            tcc_error(s1, "neither offset nor shift allowed with 'strex'");
            return;
        } else if (opcode & ENCODE_IMMEDIATE_FLAG) { // if set, it means it's NOT immediate
            tcc_error(s1, "offset not allowed with 'strex'");
            return;
        }
        if ((opcode & (1 << 24)) == 0) { // add offset after transfer
            tcc_error(s1, "adding offset after transfer not allowed with 'strex'");
            return;
        }
    ...
    case TOK_ASM_ldrexeq:
        if ((opcode & 0xFFF) || nb_shift) {
            tcc_error(s1, "neither offset nor shift allowed with 'ldrex'");
            return;
        } else if (opcode & ENCODE_IMMEDIATE_FLAG) { // if set, it means it's NOT immediate
            tcc_error(s1, "offset not allowed with 'ldrex'");
            return;
        }
        if ((opcode & (1 << 24)) == 0) { // add offset after transfer
            tcc_error(s1, "adding offset after transfer not allowed with 'ldrex'");
            return;
        }
        opcode |= 1 << 20; // L
        opcode |= 0x00f;
        ...
        break;
    default:
        expect(s1, "data transfer instruction");
    }
}
...
static void asm_emit_coprocessor_data_transfer(TCCState *s1, uint32_t high_nibble, uint8_t cp_number, uint8_t CRd, const Operand* Rn, const Operand* offset, int offset_minus, int preincrement, int writeback, int long_transfer, int load) {
    uint32_t opcode = 0x0;
    opcode |= 1 << 26; // Load/Store
    ...
    if (offset->type == OP_IM8 || offset->type == OP_IM8N || offset->type == OP_IM32) {
        ...
        if (offset->type == OP_IM8N || v < 0)
            ...
            opcode |= 1 << 23; // up
        if (v & 3) {
            tcc_error(s1, "immediate offset must be a multiple of 4");
            return;
        }
        v >>= 2;
        if (v > 255) {
            tcc_error(s1, "immediate offset must be between -1020 and 1020");
            return;
        }
        opcode |= v;
        ...
        opcode |= ENCODE_IMMEDIATE_FLAG; /* if set, it means it's NOT immediate */
        opcode |= offset->reg;
        tcc_error(s1, "Using register offset to register address is not possible here");
        return;
    } else if (offset->type == OP_VREG64) {
        opcode |= 16;
        opcode |= offset->reg;
    } else
        expect(s1, "immediate or register");
...
static void asm_coprocessor_data_transfer_opcode(TCCState *s1, int token)
{
    ...
    if (s1->tok >= TOK_ASM_c0 && s1->tok <= TOK_ASM_c15) {
        coprocessor_destination_register = s1->tok - TOK_ASM_c0;
        next(s1);
    } else {
        expect(s1, "'c<number>'");
        return;
    ...
    if (s1->tok == ',') {
        ...
        if (ops[2].type == OP_REG32) {
            ...
            tcc_error(s1, "'%s' does not support VFP register operand", get_tok_str(s1, token, NULL));
            return;
        }
    } else {
        // end of input expression in brackets--assume 0 offset
    ...
    if (token == TOK_ASM_ldc2 || token == TOK_ASM_stc2 || token == TOK_ASM_ldc2l || token == TOK_ASM_stc2l) {
        ...
        case TOK_ASM_ldc2l:
            ...
            /* fallthrough */
        case TOK_ASM_ldc2:
            asm_emit_coprocessor_data_transfer(s1, 0xF, coprocessor, coprocessor_destination_register, &ops[1], &ops[2], op2_minus, preincrement, exclam, long_transfer, 1);
            break;
        case TOK_ASM_stc2l:
            long_transfer = 1; // long transfer
            /* fallthrough */
        case TOK_ASM_stc2:
            asm_emit_coprocessor_data_transfer(s1, 0xF, coprocessor, coprocessor_destination_register, &ops[1], &ops[2], op2_minus, preincrement, exclam, long_transfer, 0);
            break;
        }
    ...
    case TOK_ASM_stcleq:
        ...
        /* fallthrough */
    case TOK_ASM_stceq:
        asm_emit_coprocessor_data_transfer(s1, condition_code_of_token(s1, token), coprocessor, coprocessor_destination_register, &ops[1], &ops[2], op2_minus, preincrement, exclam, long_transfer, 0);
        break;
    case TOK_ASM_ldcleq:
        long_transfer = 1;
        /* fallthrough */
    case TOK_ASM_ldceq:
        asm_emit_coprocessor_data_transfer(s1, condition_code_of_token(s1, token), coprocessor, coprocessor_destination_register, &ops[1], &ops[2], op2_minus, preincrement, exclam, long_transfer, 1);
        break;
    default:
        expect(s1, "coprocessor data transfer instruction");
    }
}
...
static void asm_floating_point_single_data_transfer_opcode(TCCState *s1, int token)
{
    ...
    if (ops[0].type == OP_VREG32) {
        ...
        expect(s1, "floating point register");
        return;
    }
    ...
    switch (ARM_INSTRUCTION_GROUP(token)) {
    case TOK_ASM_vldreq:
        asm_emit_coprocessor_data_transfer(s1, condition_code_of_token(s1, token), coprocessor, coprocessor_destination_register, &ops[1], &ops[2], 0, 1, 0, long_transfer, 1);
        break;
    case TOK_ASM_vstreq:
        asm_emit_coprocessor_data_transfer(s1, condition_code_of_token(s1, token), coprocessor, coprocessor_destination_register, &ops[1], &ops[2], 0, 1, 0, long_transfer, 0);
        break;
    default:
        expect(s1, "floating point data transfer instruction");
    }
}
...
static void asm_floating_point_block_data_transfer_opcode(TCCState *s1, int token)
{
    ...
    next(s1); // skip '{'
    first_regset_register = asm_parse_vfp_regvar(s1->tok, 1);
    if ((first_regset_register = asm_parse_vfp_regvar(s1->tok, 1)) != -1) {
        coprocessor = CP_DOUBLE_PRECISION_FLOAT;
        next(s1);
    } else if ((first_regset_register = asm_parse_vfp_regvar(s1->tok, 0)) != -1) {
        coprocessor = CP_SINGLE_PRECISION_FLOAT;
        next(s1);
    } else {
        expect(s1, "floating-point register");
    ...
    if (s1->tok == '-') {
        next(s1);
        if ((last_regset_register = asm_parse_vfp_regvar(s1->tok, coprocessor == CP_DOUBLE_PRECISION_FLOAT)) != -1)
            next(s1);
        else {
            expect(s1, "floating-point register");
            return;
    ...
    if (last_regset_register < first_regset_register) {
        tcc_error(s1, "registers will be processed in ascending order by hardware--but are not specified in ascending order here");
        return;
    }
    if (s1->tok != '}') {
        expect(s1, "'}'");
    ...
    case TOK_ASM_vstmdbeq: // pre-decrement store
        ...
        break;
    default:
        expect(s1, "floating point block data transfer instruction");
        return;
    }
    if (ops[0].type != OP_REG32)
        expect(s1, "(first operand) register");
        ...
        tcc_error(s1, "first operand of '%s' should have an exclamation mark", get_tok_str(s1, token, NULL));
    else
        asm_emit_coprocessor_data_transfer(s1, condition_code_of_token(s1, token), coprocessor, first_regset_register, &ops[0], &offset, 0, preincrement, op0_exclam, extra_register_bit, load);
}

#define VMOV_FRACTIONAL_DIGITS 7
#define VMOV_ONE 10000000 /* pow(10, VMOV_FRACTIONAL_DIGITS) */

static uint32_t vmov_parse_fractional_part(TCCState *s1, const char* s)
{
    uint32_t result = 0;
    int i;
...
static uint32_t vmov_parse_immediate_value(TCCState *s1) {
    uint32_t value;
    unsigned long integral_value;
    const char *p;

    if (s1->tok != TOK_PPNUM) {
        expect(s1, "immediate value");
        return 0;
    }
    p = s1->tokc.str.data;
    errno = 0;
    ...
    if (errno || integral_value >= 32) {
        tcc_error(s1, "invalid floating-point immediate value");
        return 0;
    }
    ...
    if (*p == '.') {
        ++p;
        value += vmov_parse_fractional_part(s1, p);
    }
    next(s1);
    return value;
}

static uint8_t vmov_encode_immediate_value(TCCState *s1, uint32_t value)
{
    uint32_t limit;
    uint32_t end = 0;
    ...
    if (r == -1 || value < beginning || value > end) {
        tcc_error(s1, "invalid decimal number for vmov: %d", value);
        return 0;
    }
...
static void asm_floating_point_immediate_data_processing_opcode_tail(TCCState *s1, int token, uint8_t coprocessor, uint8_t CRd) {
    ...
    immediate_value = vmov_parse_immediate_value(s1);
    ...
    case TOK_ASM_vmoveq_f64:
        ...
        if (op_minus)
            ...
            operands[1] = 0x0;
        code = vmov_encode_immediate_value(s1, immediate_value);
        operands[1] |= code >> 4;
        operands[2] = code & 0xF;
        break;
    default:
        expect(s1, "known floating point with immediate instruction");
        return;
    }
    ...
    asm_emit_coprocessor_opcode(s1, condition_code_of_token(s1, token), coprocessor, opcode1, operands[0], operands[1], operands[2], opcode2, 0);
}
...
static void asm_floating_point_reg_arm_reg_transfer_opcode_tail(TCCState *s1, int token, int coprocessor, int nb_arm_regs, int nb_ops, Operand ops[3]) {
    ...
    case CP_SINGLE_PRECISION_FLOAT:
        ...
        if (nb_ops != 2 || nb_arm_regs != 1) {
            tcc_error(s1, "vmov.f32 only implemented for one VFP register operand and one ARM register operands");
            return;
        }
        ...
        asm_emit_coprocessor_opcode(s1, condition_code_of_token(s1, token), coprocessor, opcode1, ops[0].reg, (ops[1].type == OP_IM8) ? ops[1].e.v : ops[1].reg, 0x10, opcode2, 0);
        break;
    case CP_DOUBLE_PRECISION_FLOAT:
        if (nb_ops != 3 || nb_arm_regs != 2) {
            tcc_error(s1, "vmov.f32 only implemented for one VFP register operand and two ARM register operands");
            return;
        }
        ...
        if (ops[0].type == OP_VREG64) {
            if (ops[2].type == OP_REG32) {
                ...
                tcc_error(s1, "vmov.f64 only implemented for one VFP register operand and two ARM register operands");
                return;
            }
        ...
        asm_emit_coprocessor_data_transfer(s1, condition_code_of_token(s1, token), coprocessor, ops[0].reg, &ops[1], &ops[2], 0, 0, 0, 1, opcode1);
        break;
    default:
        tcc_internal_error(s1, "unknown coprocessor");
    }
}
...
static void asm_floating_point_vcvt_data_processing_opcode(TCCState *s1, int token) {
    ...
    case TOK_ASM_vcvteq_f64_f32:
       ...
       break;
    default:
       tcc_error(s1, "Unknown coprocessor for instruction '%s'", get_tok_str(s1, token, NULL));
       return;
    }
    ...
    asm_emit_coprocessor_opcode(s1, condition_code_of_token(s1, token), coprocessor, opcode1, ops[0].reg, (ops[1].type == OP_IM8) ? ops[1].e.v : ops[1].reg, (ops[2].type == OP_IM8) ? ops[2].e.v : ops[2].reg, opcode2, 0);
}
...
*/
    ...
    for (nb_ops = 0; nb_ops < 3; ) {
        ...
        if (nb_ops == 1 && (s1->tok == '#' || s1->tok == '$' || s1->tok == TOK_PPNUM || s1->tok == '-')) {
    ...
    if (nb_arm_regs == 0) {
        ...
        if (nb_ops < 3) {
            tcc_error(s1, "Not enough operands for '%s' (%u)", get_tok_str(s1, token, NULL), nb_ops);
            return;
        }
    }
    ...
    case TOK_ASM_vmoveq_f64:
        ...
        break;
    default:
        expect(s1, "known floating point instruction");
        return;
    }
...
static void asm_floating_point_status_register_opcode(TCCState* s1, int token)
{
    ...
    case TOK_ASM_vmrseq:
        ...
        if (s1->tok != ',')
            ...
            next(s1); // skip ','
        vfp_sys_reg = asm_parse_vfp_status_regvar(s1->tok);
        next(s1); // skip vfp sys reg
        if (arm_operand.type == OP_REG32 && arm_operand.reg == 15 && vfp_sys_reg != 1) {
            tcc_error(s1, "'%s' only supports the variant 'vmrs apsr_nzcv, fpscr' here", get_tok_str(s1, token, NULL));
            return;
        }
        break;
    case TOK_ASM_vmsreq:
        ...
        break;
    default:
        expect(s1, "floating point status register instruction");
        return;
    }
    if (vfp_sys_reg == -1) {
        expect(s1, "VFP system register");
        return;
    }
    if (arm_operand.type != OP_REG32) {
        expect(s1, "ARM register");
        return;
    }
    asm_emit_coprocessor_opcode(s1, condition_code_of_token(s1, token), coprocessor, opcode, arm_operand.reg, vfp_sys_reg, 0x10, 0, 0);
}
...
static void asm_misc_single_data_transfer_opcode(TCCState *s1, int token)
{
    ...
    if (exclam) {
        if ((opcode & (1 << 24)) == 0) {
            tcc_error(s1, "result of '%s' would be unpredictable here", get_tok_str(s1, token, NULL));
            return;
        }
        opcode |= 1 << 21; // write offset back into register
    }
...
static uint32_t encbranchoffset(TCCState *s1, int pos, int addr, int fail)
{
  addr-=pos+8;
  addr/=4;
  if(addr>=0x7fffff || addr<-0x800000) {
    if(fail)
      tcc_error(s1, "branch offset is too far");
    return 0;
  }
...
static void asm_branch_opcode(TCCState *s1, int token)
{
    ...
    case TOK_ASM_bleq:
        ...
        if (!esym || esym->st_shndx != cur_text_section->sh_num) {
            tcc_error(s1, "invalid branch target");
            return;
        }
        jmp_disp = encbranchoffset(s1, s1->ind, e.v + esym->st_value, 1);
        break;
    default:
        parse_operand(s1, &op);
        break;
    ...
    switch (ARM_INSTRUCTION_GROUP(token)) {
    case TOK_ASM_beq:
        asm_emit_opcode(s1, token, (0xa << 24) | (jmp_disp & 0xffffff));
        break;
    case TOK_ASM_bleq:
        asm_emit_opcode(s1, token, (0xb << 24) | (jmp_disp & 0xffffff));
        break;
    case TOK_ASM_bxeq:
        if (op.type != OP_REG32)
            expect(s1, "register");
        else
            asm_emit_opcode(s1, token, (0x12fff1 << 4) | op.reg);
        break;
    case TOK_ASM_blxeq:
        if (op.type != OP_REG32)
            expect(s1, "register");
        else
            asm_emit_opcode(s1, token, (0x12fff3 << 4) | op.reg);
        break;
    default:
        expect(s1, "branch instruction");
    }
}
...
ST_FUNC void subst_asm_operand(TCCState *s1, CString *add_str, SValue *sv, int modifier)
{
    ...
    if ((r & VT_VALMASK) == VT_CONST) {
      ...
      no_offset:;
        ...
        reg = r & VT_VALMASK;
        if (reg >= VT_CONST)
            tcc_internal_error(s1, "");
        snprintf(buf, sizeof(buf), "[%s]",
                 get_tok_str(s1, TOK_ASM_r0 + reg, NULL));
        cstr_cat(s1, add_str, buf, -1);
    } else {
        /* register case */
        reg = r & VT_VALMASK;
                          ...
                          int out_reg)
{
    ...
    if (!is_output) { // prolog
        ...
        if (saved_regset)
            gen_le32(s1, 0xe92d0000 | saved_regset); // push {...}
        ...
        for(i = 0; i < nb_operands; i++) {
            ...
            if (op->reg >= 0) {
                if ((op->vt->r & VT_VALMASK) == VT_LLOCAL &&
                    ...
                    if (op->is_llong)
                        tcc_error(s1, "long long not implemented");
                }
            }
        }
    } else { // epilog
        ...
        if (saved_regset)
            gen_le32(s1, 0xe8bd0000 | saved_regset); // pop {...}
    }
}
    ...
    for (i = 0; i < nb_operands; i++) {
        ...
        if (op->reg >= 0) {
            if (is_reg_allocated(op->reg))
                tcc_error
                    (s1, "asm regvar requests register that's taken already");
            reg = op->reg;
            goto reg_found;
        }
      try_next:
        ...
        case '&': // Operand is clobbered before the instruction is done using the input operands
            if (j >= nb_outputs)
                tcc_error(s1, "'%c' modifier can only be applied to outputs",
                          c);
            reg_mask = REG_IN_MASK | REG_OUT_MASK;
            goto try_next;
...
ST_FUNC void asm_clobber(TCCState *s1, uint8_t *clobber_regs, const char *str)
{
    ...
    reg = asm_parse_regvar(s1, ts->tok);
    if (reg == -1) {

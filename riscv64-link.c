...
ST_FUNC void relocate_plt(TCCState *s1)
{
    ...
    if (p < p_end) {
        ...
        if ((off + ((uint32_t)1 << 20)) >> 21)
            tcc_error(s1, "Failed relocating PLT (off=0x%lx, got=0x%lx, plt=0x%lx)", (long)off, (long)got, (long)plt);
        ...
        while (p < p_end) {
            ...
            if ((off + ((uint32_t)1 << 20)) >> 21)
                tcc_error(s1, "Failed relocating PLT (off=0x%lx, addr=0x%lx, pc=0x%lx)", (long)off, (long)addr, (long)pc);
              ...
              addr_t addr, addr_t val)
{
    ...
    case R_RISCV_BRANCH:
        ...
        if ((off64 + (1 << 12)) & ~(uint64_t)0x1ffe)
          tcc_error(s1, "R_RISCV_BRANCH relocation failed"
    ...
    case R_RISCV_JAL:
        ...
        if ((off64 + (1 << 21)) & ~(((uint64_t)1 << 22) - 2))
          tcc_error(s1, "R_RISCV_JAL relocation failed"
                    " (val=%lx, addr=%lx)", (long)val, (long)addr);
        off32 = off64;
    ...
    case R_RISCV_PCREL_HI20:
        ...
        if ((off64 + ((uint64_t)1 << 20)) >> 21)
          tcc_error(s1, "R_RISCV_PCREL_HI20 relocation failed: off=%lx cond=%lx sym=%s",
    ...
    case R_RISCV_GOT_HI20:
        ...
        off64 = (int64_t)(val - addr + 0x800) >> 12;
        if ((off64 + ((uint64_t)1 << 20)) >> 21)
          tcc_error(s1, "R_RISCV_GOT_HI20 relocation failed");
        last_hi.addr = addr;
        last_hi.val = val;
        write32le(ptr, (read32le(ptr) & 0xfff)
                       | ((off64 & 0xfffff) << 12));
...
#endif
        if (val != last_hi.addr)
          tcc_error(s1, "unsupported hi/lo pcrel reloc scheme");
        val = last_hi.val;
        addr = last_hi.addr;
    ...
    case R_RISCV_RVC_BRANCH:
        ...
        if ((off64 + (1 << 8)) & ~(uint64_t)0x1fe)
          tcc_error(s1, "R_RISCV_RVC_BRANCH relocation failed"
                    " (val=%lx, addr=%lx)", (long)val, (long)addr);
        off32 = off64;
